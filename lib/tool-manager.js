...
const checkCliInstalled = (pkg) => {
  try {
    ...
    if (bin) {
      ...
      console.log(`[tool] CLI ${pkg} (${bin}): FOUND`);
      return true;
    }
  } catch (err) {
    console.log(`[tool] CLI ${pkg}: NOT found`);
  }
  return false;
};
...
const getCliVersion = (pkg) => {
  try {
    ...
    const bin = binMap[pkg];
    if (bin) {
      // Use shorter timeout - we already know the binary exists from checkCliInstalled
      const out = execSync(`${bin} --version`, { stdio: 'pipe', timeout: 3000, encoding: 'utf8' });
      const match = out.match(/(\d+\.\d+\.\d+)/);
      if (match) {
        console.log(`[tool] CLI ${pkg}: v${match[1]}`);
        return match[1];
      } else {
        // CLI exists but version format not recognized - mark as installed
        console.log(`[tool] CLI ${pkg}: installed (version unrecognized: ${out.trim().substring(0, 30)})`);
        return 'installed';
      }
    }
  } catch (err) {
    // If version detection fails, just mark as installed (we already know the binary exists)
    console.log(`[tool] CLI ${pkg}: installed (version check failed)`);
  }
  return 'installed';
};
