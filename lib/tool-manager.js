...
const getPublishedVersion = async (pkg) => {
  try {
    ...
    if (cached && Date.now() - cached.timestamp < 3600000) {
      console.log(`[tool-manager] getPublishedVersion(${pkg}): cached = ${cached.version}`);
    ...
    const result = await new Promise((resolve) => {
      ...
      const timer = setTimeout(() => {
        ...
        console.log(`[tool-manager] npm view ${pkg} TIMEOUT`);
        resolve(null);
      }, 10000);
      proc.on('close', (code) => {
        clearTimeout(timer);
        console.log(`[tool-manager] npm view ${pkg} finished: code=${code}, stdout="${stdout.trim()}"`);
        resolve(code === 0 ? stdout.trim() : null);
      });
      proc.on('error', (err) => {
        clearTimeout(timer);
        console.log(`[tool-manager] npm view ${pkg} error:`, err.message);
        resolve(null);
      });
    });
    ...
    if (result) {
      ...
      console.log(`[tool-manager] getPublishedVersion(${pkg}): result = ${result}`);
    } else {
      console.log(`[tool-manager] getPublishedVersion(${pkg}): null result`);
    }
    return result;
  } catch (err) {
    console.log(`[tool-manager] getPublishedVersion(${pkg}): error =`, err.message);
    return null;
  }
};
...
const getCliVersion = (pkg) => {
  try {
    ...
    const bin = binMap[pkg];
    if (bin) {
      // Use longer timeout - some CLI tools are slow to respond
      const out = execSync(`${bin} --version`, { stdio: 'pipe', timeout: 10000, encoding: 'utf8' });
      const match = out.match(/(\d+\.\d+\.\d+)/);
      if (match) {
        console.log(`[tool-manager] CLI ${pkg} (${bin}) version: ${match[1]}`);
        return match[1];
      } else {
        // CLI exists but version format not recognized - mark as installed
        console.log(`[tool-manager] CLI ${pkg} (${bin}) installed (version format unrecognized)`);
        return 'installed';
      }
    }
  } catch (err) {
    const isTimeout = err.killed || err.code === 'ETIMEDOUT';
    console.log(`[tool-manager] CLI ${pkg}: ${isTimeout ? 'timeout' : 'error'} getting version`);
  }
  return null;
};
...
const checkToolViaBunx = async (pkg, pluginId = null, category = 'plugin', frameWork = null) => {
  try {
    ...
    console.log(`[tool-manager] checkToolViaBunx: ${pkg} (category=${category}, isCli=${isCli})`);
    const installed = isCli ? checkCliInstalled(pkg) : checkToolInstalled(pluginId || pkg, frameWork);
    console.log(`[tool-manager]   installed=${installed}`);
    const installedVersion = isCli ? getCliVersion(pkg) : getInstalledVersion(pkg, pluginId, frameWork);
    console.log(`[tool-manager]   installedVersion=${installedVersion}`);
    const publishedVersion = await getPublishedVersion(pkg);
    console.log(`[tool-manager]   publishedVersion=${publishedVersion}`);
